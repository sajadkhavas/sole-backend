...
Route::prefix('v1')->name('api.v1.')->group(function (): void {
    ...
    Route::post('/commerce/shipping/provider-events', [CommerceLifecycleController::class, 'shippingWebhook'])->middleware('throttle:p07-shipping-webhook')->name('commerce.shipping.provider-events');
    ...
    Route::middleware(['auth:sanctum', 'throttle:120,1'])->group(function (): void {
        ...
        Route::get('/observability/consent', [ObservabilityController::class, 'consentStatus'])->middleware('throttle:p11-experiments')->name('observability.consent.show');
        Route::put('/observability/consent', [ObservabilityController::class, 'consent'])->middleware('throttle:p11-experiments')->name('observability.consent.update');
        Route::post('/observability/events', [ObservabilityController::class, 'event'])->middleware('throttle:p11-analytics')->name('observability.events.store');
        Route::get('/observability/experiments', [ObservabilityController::class, 'experiments'])->middleware('throttle:p11-experiments')->name('observability.experiments.index');
        Route::post('/observability/experiments/exposure', [ObservabilityController::class, 'exposure'])->middleware('throttle:p11-experiments')->name('observability.experiments.exposure');
        ...
        Route::post('/customer/otp', [OtpController::class, 'request'])->middleware('throttle:20,1')->name('customer.otp.request');
        Route::post('/customer/otp/verify', [OtpController::class, 'verify'])->middleware('throttle:30,1')->name('customer.otp.verify');
    });
});
